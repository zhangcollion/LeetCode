...
class Memory(nn.Module):
    def __init__(self, n_nodes, memory_dimension, input_dimension, 
                message_dimension=None, device='cpu', combination_method='sum'):
    ...
    def store_raw_messages(self, nodes, node_id_to_messages):
        for node in nodes:
            self.messages[node].extend(l[node])

    def get_memory(self, node_idxs, values):
    ...
    def detach_memory(self):
        ...
        for k, v in self.messages.items():
            new_node_messages = []
            for message in v:
                new_node_messages.append((message[0].detach_(), message[1]))
